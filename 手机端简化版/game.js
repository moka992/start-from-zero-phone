...
const DETAILED_MECHANICS_HTML = [
  '<strong>1. 游戏目标与流程</strong>',
  '目标：平衡“现金、口碑、销量、生命周期”，尽量长期存活并持续换代。',
  '流程：随机市场区域 -> 手机设计 -> 运营推进（月度） -> 产品停产/下一代/企业结算。',
  '起始资金：¥10,000,000；默认单代生命周期上限：30 个月。',
  '失败判定（主要）：现金流断裂、口碑崩盘、设计无法立项、致命黑天鹅。',
  '',
  '<strong>2. 手机设计要素（作用）</strong>',
  'SoC：影响性能跑分、散热压力、成本与极客吸引度。',
  '屏幕：材质/尺寸/比例/形态/特性共同影响成本、功耗、观感、线上线下偏好与口碑。',
  '摄像头系统：主摄/超广/黑白/长焦/前摄影响影像分、体积、重量、需求标签。',
  '机身材质与三围：影响体积可行性、散热、重量、审美偏好和渠道接受度。',
  '电池与散热：决定续航评测、热压力、性能释放稳定性。',
  'SKU（内存/存储/配比）：影响成本结构、受众覆盖、库存结构和补货策略（配比总和必须 100%）。',
  '额外功能：按年份解锁，影响成本/体积/可靠性/热度及需求侧偏好。',
  '',
  '<strong>2.1 部件影响量化速查（销量/成本/口碑）</strong>',
  'SoC/影像/屏幕/存储会进入质量分：质量分 = 0.34*SoC + 0.20*影像 + 0.18*屏幕 + 0.14*(内存+存储) + 机身/功能等项（总分 15~100）。质量分越高，销量与口碑越稳。',
  '极客吸引度（0~100）核心权重：SoC 约 40%、影像约 24%、屏幕形态+功能约 32%，会直接放大线上需求。',
  '无前摄销量惩罚：影像需求系数约 ×0.78；无主摄约 ×0.58；如果全机无摄像头，约 ×0.22（强惩罚）。',
  '边框/观感：边框过宽会把观感系数压低到约 0.68；窄边框可到约 1.08，线下门店转化更友好。',
  '散热：热压力越高，热相关需求系数会从约 1.03 下降到最低约 0.74；同时质量分最多额外扣约 9 分（影响口碑与复购）。',
  '折叠屏渠道增益：线上约 ×1.06~1.22，线下约 ×1.14~1.36（随尺寸等因素变化）。',
  '材质渠道偏好：芳纶对线上约 ×1.10，陶瓷对线下约 ×1.10，木质对线上线下约 ×1.08。',
  '线下偏好点数：护眼/LTPO/P3/磁吸/无线充/快充/GPS 等每项累积，线下偏好最终可提升到约 ×1.28 上限。',
  '成本传导：单台制造成本 =（元器件 + 组装 + 物流）×市场成本系数；其中组装成本=80+元器件成本*8.5%。采购档位会直接改BOM（现货 1.08、季度 1.00、年度 0.92）。',
  '首发口碑变化（立项当月）：受市场/营销/质量分共同影响，质量分每高 1 分，首发口碑增量约额外 +0.08（再叠加难度修正）。',
  '',
  '<strong>3. 市场与环境指标</strong>',
  '成立区域：影响人口规模、线上渗透、供应链/研发/线下成熟度、人力与房租等环境参数。',
  '随机市场环境（Buff/Debuff）：改变需求、成本与口碑偏移，决定本局外部大盘。',
  '线上/线下并行：不同设计点会对线上与线下需求产生不同强度影响。',
  '',
  '<strong>4. 事件系统影响</strong>',
  '普通事件：月度波动，影响需求、口碑、资金与补货节奏。',
  '黑天鹅：高冲击风险事件，可能导致现金流恶化、供应周期拉长、需求坍塌甚至直接结算。',
  '强 Buff：如顶流评测、报道、资本关注等，可显著拉升热度与口碑。',
  '',
  '<strong>5. 需求算法（含关键阈值）</strong>',
  '需求是多因子乘积，不是纯随机：区域基底 × 设计匹配 × 定价可信度 × 渠道偏好 × 生命周期 × 事件扰动。',
  '历史阶段修正（基础需求）：2014-2016 约 ×4.2，2017-2019 约 ×3.2，2020-2023 约 ×2.3，2024+ 回归常规模型。',
  '价格带偏好：2016 及前偏 200~2000；2017-2021 偏 2000~3500；2022+ 偏 3000~4000（低价盘仍有更大基本盘）。',
  '高价低配惩罚（按售价/成本）：口碑<=75 时 1.75 倍起线性惩罚、3.0 倍后指数下滑；口碑>75 时阈值放宽到 2.0 与 3.25；口碑>90 可免疫该惩罚。',
  '性能匹配门槛：当售价高于“当前可选最高成本参考”阈值（真实 1.15x / 困难 1.10x）后，跑分匹配会显著影响需求。',
  '性价比旗舰加成：中高端及以上 SoC + 中档及以上存储组合，且售价/成本<2.0 可触发增益，<=1.3 时上限约 +38%。',
  '',
  '<strong>6. 运营与结算规则</strong>',
  '推进月份：消化库存并产生现金、毛利、口碑变化和事件反馈。',
  '补货：受现金、采购方案、交期影响；有在途库存，支持跨月累积与分批入库。',
  '预订机制：前 2 个月缺货可转预订，预订款按 100% 立即入账（用于缓解冷启动现金流）。',
  '代际贷款：每代可选 100 万，年化 10%，6 个月后扣回本息。',
  '停产/换代：旧机进入清库存，新机重开生命周期；企业指标持续累计。',
  '',
  '<strong>7. 难度说明（真实 vs 困难）</strong>',
  '真实：基础需求系数 1.4，品牌爬坡与容错更友好，适合经营与换代练习。',
  '困难：基础需求系数 0.75，设计弹性与波动惩罚更强，崩盘概率更高。',
  '一句话：真实更看策略，困难既看策略也看抗风险能力。',
  '',
  '<strong>8. 名词速查</strong>',
  '在手库存：当前可卖数量；在途库存：已补货未到货数量。',
  '口碑：品牌信任与复购意愿的核心指标，会影响定价容忍度与需求。',
  '生命周期：爬坡/成熟/衰退动态阶段，不同阶段需求趋势不同。',
  '综合跑分：性能、影像、屏幕、存储、续航的统一评价面板，用于辅助判断，不是唯一决策依据。',
  '<span class="muted">注：以上数值为当前版本核心规则，后续版本可能小幅调参。</span>'
].join('<br>');
